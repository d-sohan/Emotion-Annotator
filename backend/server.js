...
app.post('/image/upload', imgUpload.single('img'), (req, res) => {
...
app.post('/image/process', express.json(), (req, res) => {
  ...
  imgCompute(temp);
  imgAnnotate(temp, name, annotation)
    .then((message) => {
      res.send('done');
    })
    .catch((message) => {
...
app.post('/image/download', express.json(), (req, res) => {
  ...
  const outputPath = __dirname + '/output/image/' + temp + '/' + csvFileName + '_annotated.csv';
